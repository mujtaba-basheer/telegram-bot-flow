...
export const authorizationUrl = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const { __ms, memberstack } = req.body as RequestBody;

    const baseUrl = "https://twitter.com/i/oauth2/authorize";
    try {
      const qs = await getAuthorizationParamsString(
        api_scope,
        __ms,
        memberstack
      );
...
export const getLoginDetails = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { state } = req.params;

      const details = findLoginDetails(state);
      if (details) {
        res.json({
          status: true,
          data: details,
        });
      } else return next(new AppError("Login Details Not Found", 404));
    } catch (error) {
      return next(new AppError(error.message, 501));
    }
  }
