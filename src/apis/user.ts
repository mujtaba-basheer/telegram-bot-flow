              ...
              (resp) => {
                ...
                resp.on("end", () => {
                  ...
                  for (const tweet of tweets) {
                    ...
                    tweet.created_at = { date: "", time: "", o: d } as
                      | string & { date: string; time: string; o: Date };
                    const dateArr = d
  ...
  try {
    ...
    dynamodb.getItem(getUserParams, async (err, data) => {
      ...
      if (limit_o) {
        ...
        const scanItems: (
          getTweetsParams: AWS.DynamoDB.ScanInput
        ...
        while (true) {
          try {
            const scanResult = await scanItems(getTweetsParams);
            tweets = tweets.concat(scanResult.Items);

            if (scanResult.LastEvaluatedKey)
              getTweetsParams.ExclusiveStartKey = scanResult.LastEvaluatedKey;
            else break;
          } catch (error) {
        ...
        tweets.sort((t1, t2) => {
          try {
            const d1 = Number(t1.id.S.split(".")[1]);
            const d2 = Number(t2.id.S.split(".")[1]);
        ...
        const ids: string[] = tweets.map((x) => x.id.S);
        ...
        const request = https.request(
          ...
          (resp) => {
            ...
            resp.on("end", () => {
              ...
              /*
                tweets.sort((t1, t2) => {
                  const c1 = t1.created_at.o;
                  const c2 = t2.created_at.o;
